...
class HeatCapacity:
    ...
    def GetTask(self):
        ...
        if taskType == 1:
            self.taskString += f"Деталь механизма массой {self.VALUES.m} нагревают от {self.VALUES.t1}" + f"\n , до {self.VALUES.t2}. Какова теплоемкость детали, если выделилось {self.VALUES.Q} тепла?"
            self. ANNOTATIONS.append(str(self.VALUES.c))
        elif taskType == 2:
            self.taskString += f"При нагреве детали выделилось {self.VALUES.Q} тепла" + f"\n , масса детали {self.VALUES.m}, а ее теплоемкость {self.VALUES.c}. На сколько изменились температура детали?"
